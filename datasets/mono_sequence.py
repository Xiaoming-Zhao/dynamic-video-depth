...
class Dataset(base_dataset):
    ...
    def add_arguments(cls, parser):
        ...
        parser.add_argument('--mono_data_root', type=str)
        return parser, set()
    ...
    def __init__(self, opt, mode='train', model=None):
        ...
        data_root = self.opt.mono_data_root
        ...
        if mode == 'train':
            ...
            if self.opt.subsample:
                data_path = join(data_root, seq_prefix, 'subsample')
            else:
                data_path = join(data_root, seq_prefix, '%03d' % 1)
            ...
            frame_data_path = join(data_root, frame_prefix)
            self.n_frames = len(sorted(glob(join(frame_data_path, '*.npz')))) + 0.0

        else:
            data_path = join(data_root, frame_prefix)
    ...
    def __getitem__(self, idx):
        ...
        # Used for aligning predicted depth to MVS depth
        if self.opt.follow_dynibar:
            assert "disp_scale" in dataset
            assert "disp_shift" in dataset
            sample_loaded["disp_scale"] = dataset["disp_scale"]
            sample_loaded["disp_shift"] = dataset["disp_shift"]

            if "depth_pred_1" in sample_loaded:
                sample_loaded['depth_pred_1'] = self._rescale_depth(np.copy(sample_loaded['depth_pred_1']), sample_loaded["disp_scale"], sample_loaded["disp_shift"], sample_loaded["depth_is_disp"])

        self.convert_to_float32(sample_loaded)

        return sample_loaded
    
    def _rescale_depth(self, depth, disp_scale, disp_shift, depth_is_disp, tiny_val=1e-8):
        if depth_is_disp.bool():
            disp = depth
        else:
            disp = 1 / (depth + tiny_val)

        disp_rescale = np.clip(disp_scale * disp + disp_shift, tiny_val, None)

        depth_rescale = 1 / (disp_rescale + tiny_val)
