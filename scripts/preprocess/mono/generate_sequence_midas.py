...
@lru_cache(maxsize=1024)
def read_flow_data(flow_root, frame_id_1, frame_id_2):
    data = np.load(
        flow_root / f"flowpair_{frame_id_1:05d}_{frame_id_2:05d}.npz",
        allow_pickle=True,
    )
    data_dict = {}
    for k in data.keys():
...
def collate_sequence_fix_gap(data_root, flow_root, seq_list, gap=1):
    sequential_pairs_start = seq_list
    sequential_pairs_end = seq_list + gap
    list_of_pairs_select = [
        (x, y) for x, y in zip(sequential_pairs_start, sequential_pairs_end)
    ]
    sequential_data = collate_pairs(data_root, flow_root, list_of_pairs_select)
...
def collate_pairs(data_root, flow_root, list_of_pairs):
    dict_of_list = {}
    for idp, pair in enumerate(list_of_pairs):
        dd = datadict_from_pair(data_root, flow_root, pair)
        for k, v in dd.items():
...
def datadict_from_pair(data_root, flow_root, pair):
    frame_id_1, frame_id_2 = pair
    im1_data = read_frame_data(data_root, pair[0])
    im2_data = read_frame_data(data_root, pair[1])
    fid_1, fid_2 = sorted([frame_id_1, frame_id_2])
    flow_data_dict = read_flow_data(flow_root, fid_1, fid_2)
    if fid_1 == frame_id_1:
    ...
    if "motion_seg" in im1_data.keys():
        motion_seg = torch.from_numpy(im1_data["motion_seg"])[
            None, ..., None, None
        ].float()
    else:
        motion_seg = mask_2
    ...
    if "disp_scale" in im1_data:
        # following DynIBaR, we estimate scale and shift to align predicted depth to MVS depth
        assert (
            im1_data["disp_scale"] == im2_data["disp_scale"]
        ), f'{im1_data["disp_scale"]}, {im2_data["disp_scale"]}'
        assert (
            im1_data["disp_shift"] == im2_data["disp_shift"]
        ), f'{im1_data["disp_shift"]}, {im2_data["disp_shift"]}'
        samples["disp_scale"] = torch.FloatTensor([float(im1_data["disp_scale"])])[
            None, :
        ]
        samples["disp_shift"] = torch.FloatTensor([float(im1_data["disp_shift"])])[
            None, :
        ]
    if "depth_is_disp" in im1_data:
        assert (
            im1_data["depth_is_disp"] == im2_data["depth_is_disp"]
        ), f'{im1_data["depth_is_disp"]}, {im2_data["depth_is_disp"]}'
        samples["depth_is_disp"] = torch.BoolTensor([bool(im1_data["depth_is_disp"])])[
            None, :
        ]
    return samples
...
if __name__ == "__main__":
    """
    conda activate gdvs_pytorch2 && \
    export PYTHONPATH=/root/code/private_dynamic-video-depth:$PYTHONPATH && \
    python /root/code/private_dynamic-video-depth/scripts/preprocess/mono/generate_sequence_midas.py \
    --data_root /root/code/private_dynamic-video-depth/datafiles/mono/processed_dynibar \
    --save_dir /root/code/private_dynamic-video-depth/datafiles/mono/processed_dynibar \
    --scene_id_list Balloon2
    """
    ...
    scene_flow_dir = data_root / "flow_pairs"

    scene_save_dir = save_dir / "sequences_select_pairs_midas" / "001"
    scene_save_dir.mkdir(parents=True, exist_ok=True)

    all_frames = sorted(
        list(scene_dir.glob("frame_*.npz")),
        key=lambda x: int(x.stem.split("frame_")[1]),
    )
    ...
    for gap in tqdm.tqdm(gaps, desc="#gap"):
        fids = np.arange(len(all_frames) - bs - gap)
        cnt = 0

        for f in tqdm.tqdm(fids, desc="#frames"):
            seq_list_forward = np.arange(f, f + bs)

            sequence = collate_sequence_fix_gap(
                scene_dir,
                scene_flow_dir,
                seq_list_forward,
                gap=gap,
            ...
            torch.save(
                sequence,
                scene_save_dir / f"shuffle_False_gap_{gap:02d}_sequence_{cnt:05d}.pt",
            )
            cnt += 1
